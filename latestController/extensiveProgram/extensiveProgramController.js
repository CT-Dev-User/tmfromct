...
export const createExtensiveProgram = async (req, res) => {
  try {
    ...
    let parseCardData,
      parseHeaderData,
      parseHighlights,
      parseCourseCurriculam,
      parseTopInDemandTools,
      parseFeesStuctures;
    ...
    try {
      ...
      parseHeaderData =
        typeof headerData === "string" ? JSON.parse(headerData) : headerData;
      parseHighlights =
        typeof highlights === "string" ? JSON.parse(highlights) : highlights;
      parseCourseCurriculam =
        typeof courseCurriculam === "string"
          ? JSON.parse(courseCurriculam)
          : courseCurriculam;
      parseTopInDemandTools =
        typeof topInDemandTools === "string"
          ? JSON.parse(topInDemandTools)
          : topInDemandTools;
      parseFeesStuctures =
        typeof FeesStuctures === "string"
          ? JSON.parse(FeesStuctures)
          : FeesStuctures;
    } catch (error) {
      return res
        .status(400)
    ...
    await handleFileUploads(
      "highlightsIcon",
      parseHighlights.highlightPoints,
      "icon"
    );
    await handleFileUploads(
      "topInDemandToolsLogo",
      parseTopInDemandTools,
      "logo"
    );
    ...
    res
      .status(201)
      .json({ message: "Extensive program created", newExtensiveProgram });
  } catch (error) {
    res
      .status(500)
      .json({
        message: "Error creating Extensive program Page",
        error: error.message,
      });
  }
};
...
export const deleteExtensiveProgramPage = async (req, res) => {
  try {
    ...
    if (!page)
      return res
        .status(404)
        .json({ error: "Extensive program Page not found" });
    await extensiveProgramModel.findByIdAndDelete(id);

    res
      .status(200)
      .json({ message: "Extensive program Page deleted successfully" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
export const updateExtensiveProgram = async (req, res) => {
  try {
    ...
    const handleFileUploads1 = async (
      fileKey,
      indexKey,
      targetArray,
      propertyToUpdate
    ) => {
      if (req.files && req.files[fileKey] && req.body[indexKey]) {
        for (let i = 0; i < req.files[fileKey].length; i++) {
          const file = req.files[fileKey][i];
          const index = req.body[indexKey][i]; // Retrieve the corresponding index

          // Upload the file to Cloudinary
          const imageUrl = await uploadToCloudinary(file.path);

          // Ensure the array is updated at the correct index
          const parsedIndex = parseInt(index, 10);
          if (targetArray[parsedIndex]) {
            targetArray[parsedIndex][propertyToUpdate] = imageUrl; // Update existing object
          } else {
            targetArray[parsedIndex] = { [propertyToUpdate]: imageUrl }; // Create new object
          }
        }
      }
    };

    console.log(req.files.highlightsIcon);

    // Handle file uploads
    await handleFileUploads("headerImage", [parseHeaderData], "headerBgImage");
    await handleFileUploads1(
      "highlightsIcon",
      "highlightsIconIndex",
      parseHighlights ? parseHighlights.highlightPoints : [],
      "icon"
    );
    await handleFileUploads1(
      "topInDemandToolsLogo",
      "topInDemandToolsLogoIndex",
      parseTopInDemandTools,
      "logo"
    );
    ...
    const updateData = {
      ...
      courseCurriculam:
        parseCourseCurriculam || existingProgram.courseCurriculam,
      skillYouLearn: skillYouLearn || existingProgram.skillYouLearn,
      topInDemandTools:
        parseTopInDemandTools || existingProgram.topInDemandTools,
      FeesStuctures: parseFeesStuctures || existingProgram.FeesStuctures,
    };
    console.log(updateData);
    const updatedProgram = await extensiveProgramModel.findByIdAndUpdate(
      id,
      updateData,
      { new: true }
    );
    ...
    res
      .status(200)
      .json({ message: "Extensive program updated", updatedProgram });
  } catch (error) {
    res
      .status(500)
      .json({
        message: "Error updating Extensive program",
        error: error.message,
      });
  }
};
